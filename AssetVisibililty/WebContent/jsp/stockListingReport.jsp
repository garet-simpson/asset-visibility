...
<section id="mission" class="section">
  <div class="container">
    <div class="text-content">
      <h2 class="headline">Stock Listing Report</h2>
      <p>This report shows all the items at the selected location.</p>
      	<form action="stockListingReport" method="post">
			<label>Item Location</label> 
			<select name="location" id = "location" onchange="this.form.submit()">
				<option value = "select">Select Location</option>
					<c:forEach items="${requestScope.Locations}" var="locations" varStatus="loop">
						<option value = "${locations.destinationLocationName}" 
						<c:if test="${requestScope.Location.equals(locations.destinationLocationName)}">selected</c:if>
						>${locations.destinationLocationName}</option>
					</c:forEach>
